...
const Clock = function () {
  ...
  function addClock(city) {
    const newclock = document.createElement("world-clock");
    newclock.setAttribute("city", city);
    clockCollection[nextIndex] = newclock;
    newclock.setAttribute("id", nextIndex++);

    newclock.addEventListener("deleted", (event) => {
      const deletedItemId = event.detail.id;
      delete clockCollection[deletedItemId];
    });

    wrapper.append(newclock);
  }
...
function load(event) {
  ...
  clock.addClock("tehran");
  clock.addClock("berlin");
  clock.addClock("new_york");
  clock.addClock("tokyo");
  createCityList(timeZones);

  function timerTick() {
    for (key in clock.clockCollection) {
      clock.clockCollection[key].displayTime();
    }
  }
